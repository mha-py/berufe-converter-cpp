...
void predict_mult(std::string str, int N, Transformer &net, Tokenizer &tokenizer, std::vector<std::string> &res, std::vector<float> &ps) {
    ...
    y.push_back(tokenizer.t2i["<S>"]); // start token
    int stoptoken = tokenizer.t2i["</S>"]; // stop token

    std::cout << "start token" << y[0] << std::endl;
    std::cout << "stop token" << stoptoken << std::endl;
...
void sortCorresponding(std::vector<float> &ps, std::vector<std::string> &out) {
    ...
    std::vector<std::string> out_sorted(out.size());
